...
non-punitive — never `version=''`, and not coverage-flipping; see §1) so the
empty-version fallback can't
...
- **Binary invariant (load-bearing):** every post-v6 binary `package_hash` row
  ...
  it does **not** flip the snapshot's `coverage_complete`, because
  `coverage_complete = false` relaxes strict mode for the *whole* suite — one
  malformed stanza must not downgrade the suite's strict-mode posture (the harm
  the round-1 review flagged). The lone malformed `.deb` is left unvouched:
  reapable by GC and fail-closed under strict mode, which is safe because it is
  apt-unusable anyway. (Earlier drafts also flipped coverage-incomplete;
  reverted to skip-only after the round-1/round-5 reviews.)
...
- `internal/freshness/packages_parse.go` — parse `Version:` → `PackageRef.Version`
  (binary stanzas); `""` when a binary stanza omits `Version:`.
- `internal/freshness/adoption.go` — `buildPackageHashes` carries `version` and
  SKIPS a version-less binary stanza (binary invariant, non-punitive — no
  coverage flip; see §1). Shared by the adoption and reconcile freshness paths.
